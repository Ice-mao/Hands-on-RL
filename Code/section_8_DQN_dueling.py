...
class VAnet(torch.nn.Module):
    ...
    def __init__(self, state_dim, hidden_dim, action_dim):
        ...
        self.fc1 = torch.nn.Linear(state_dim, hidden_dim)  # 共享第一个全连接层的参数
...
class DQN:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, learning_rate, gamma,
        ...
        if dqn_type == 'DuelingDQN':  # Dueling DQN采取不一样的网络框架
            self.net = VAnet(state_dim, hidden_dim,
                               self.action_dim).to(device)
            self.target_net = VAnet(state_dim, hidden_dim,
                                      self.action_dim).to(device)
        else:
...
plt.xlabel('Episodes')
plt.ylabel('Returns')
plt.title('{} on {}'.format(agent.dqn_type, env_name))
plt.show()
